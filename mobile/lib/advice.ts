...
import { writerContext } from "@/lib/onboarding";
import { currentSettings } from "@/lib/store";
...
export function entrySummary(entry: Entry): string {
  // Who is writing, from the onboarding answers, so advice for a Prep student
  // doesn't read like advice for a Senior. Null when they said nothing, in
  // which case the summary is exactly what it was before onboarding existed.
  const who = writerContext(currentSettings());
  const parts = who ? [who, `Format: ${MODE_META[entry.mode].label}`] : [`Format: ${MODE_META[entry.mode].label}`];
...
export async function fetchEntryAdvice(
  ...
  return {
    ...
    basis: "generated by Google Gemini from this entry",
  };
}
