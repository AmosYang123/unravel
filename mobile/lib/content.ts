import { AlignLeft, Heart, HelpCircle, List, Mic, SlidersHorizontal, Zap, type LucideIcon } from "lucide-react-native";
 ...
 * `accent` colour. The web app applies this with a CSS `hue-rotate` filter, which
 * has no React Native equivalent — RN has no filter pipeline at all. `modeGlyphColor`
 * below reproduces the same *intent* (same lightness/saturation as the theme's
 * accent, hue turned by a fixed amount per mode) by converting the accent to HSL,
 * rotating the hue in that space, and converting back to a hex string RN can use
 * directly as a `color` prop. This is not bit-identical to the CSS filter (which
 * rotates in a luma-preserving RGB matrix, not plain HSL), but it keeps the same
 * design promise: every mode glyph stays exactly as light/saturated as the
 * accent the theme already uses everywhere else, just at its own hue.
...
/** The theme's own accent, rotated to this mode's hue — the RN equivalent of `modeGlyphStyle` on web. */
export function modeGlyphColor(mode: EntryMode, accentHex: string): string {
  const { h, s, l } = hexToHsl(accentHex);
  return hslToHex(h + MODE_META[mode].hueRotate, s, l);
}
...
export function recommendSongs(
  ...
  return Array.from({ length: Math.min(count, pool.length) }, (_, i) => {
    const picked = pool[(offset + i) % pool.length]!;
    return { ...picked.song, reason: picked.reason };
  });
}
