...
export interface OptionalSetupAnswers {
  name: string;
  yearLevel: YearLevel | "";
  focusAreas: string[];
  goals: string[];
...
/** Builds a complete setup update while preserving intentionally blank answers. */
export function optionalSetupPatch(answers: OptionalSetupAnswers, completedAt: string): Partial<Settings> {
  const draft = answers.interestDraft.trim();
  return {
    name: answers.name.trim(),
    yearLevel: answers.yearLevel,
    focusAreas: answers.focusAreas,
    goals: answers.goals,
    interests: draft && answers.interests.length < MAX_INTERESTS
      ? [...answers.interests, draft]
      : answers.interests,
...
export function writerContext(settings: Pick<Settings, "yearLevel" | "focusAreas">): string | null {
  ...
  if (settings.yearLevel) {
    lines.push(
      `Who is writing: a student in ${labelFor(YEAR_LEVELS, settings.yearLevel)}. Pitch the wording and the steps for that age.`,
    );
  }
